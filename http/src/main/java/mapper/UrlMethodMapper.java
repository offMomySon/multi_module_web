...
public class UrlMethodMapper {
    ...
    public UrlMethodMapper(Map<MethodIndicator, Method> values) {
        validateNull(values);
    ...
    public static UrlMethodMapper create(Class _clazz, String packageName) {
        InputStream stream = _clazz.getResourceAsStream("/" + packageName.replaceAll("[.]", "/"));
        BufferedReader reader = new BufferedReader(new InputStreamReader(stream));

        List<Class> allClasses = reader.lines()
            .filter(isClass())
            .map(parseClassName())
    ...
    public static void map(Class clazz) {
        ...
        for (Annotation annotation : annotations) {
            if (annotation instanceof RequestMapping) {
                ...
                System.out.println("value: " + Arrays.toString(myAnnotation.method()));
            }
        }
    }
    ...
    public static List<Class> findAllClasses(Class clazz, String packageName) {
        InputStream stream = clazz.getResourceAsStream("/" + packageName.replaceAll("[.]", "/"));
        BufferedReader reader = new BufferedReader(new InputStreamReader(stream));
