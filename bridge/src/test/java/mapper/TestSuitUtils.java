...
public class TestSuitUtils {
    ...
    public static Stream<Arguments> provideUrlTestSuite() {
        return Stream.of(
                "/path1, /path1, true",
                "/path1, /path2, false",
            ...
            ).map(s -> Arrays.stream(s.split(",")).map(st -> st.trim()).collect(Collectors.toUnmodifiableList()))
            .map(e -> Arguments.of(e.get(0), e.get(1), Boolean.valueOf(e.get(2))));
    }
}
